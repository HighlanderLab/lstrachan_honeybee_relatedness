...
#########################3
a40w20 <- read.csv("Apiary40_20Kw/CsdVariability.csv")
a40w20 <- a40w20[!is.na(a40w20$Rep),]
a40w20 <- a40w20[a40w20$Rep != "Rep",]


a40w20$nCSDage0 <- as.numeric(a40w20$nCSDage0)
a40w20$totalCSDage0 <- as.numeric(a40w20$totalCSDage0)
a40w20$year <- as.factor(as.numeric(a40w20$year))
a40w20$Rep <- as.factor(as.numeric(a40w20$Rep))
a40w20A <-  a40w20 %>% group_by(Rep, year) %>%  dplyr::summarise(Colony = mean(nCSDage0),
                                                                 Apiary = mean(totalCSDage0)) %>%
  pivot_longer(c(Colony, Apiary))


ggplot(data = a40w20A, aes(x = year, y = value, colour = Rep, group = Rep)) +
  geom_line() +
  scale_colour_viridis(discrete = T) +
  theme_bw() +
  ylab("Number of distinct csd alleles") + xlab("Year") +
...
#########################3
#Apiary with 20 or 40 colonies, 20K workers
#########################3
a20w20A$Scenario <- "20 colonies"
a40w20A$Scenario <- "40 colonies"
a20 <- rbind(a20w20A, a40w20A)

a20$ScenarioRep <- paste0(a20$Scenario, a20$Rep)
a20A <- a20 %>%  group_by(year, name, Scenario) %>%  summarise(value = mean(value))

png("/home/x/JANA/HoneybeeGeno/CsdVariability1.png", res=800, width=170, height=80, units="mm")
ggplot() +
  geom_line(data = a20, aes(x = year, y = value, colour = Scenario, group = ScenarioRep), alpha = 0.3) +
  geom_line(data = a20A, aes(x = year, y = value, colour = Scenario, group = Scenario), size = 1) +
  scale_colour_manual("", values = c("#95D840FF", "#33638DFF")) +
  theme_bw() +
